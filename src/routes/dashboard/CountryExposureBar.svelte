  ...
  const segments: Segment[] = $derived.by(() => {
    ...
    for (const trade of tradeResults) {
      if (trade.type === 'buy') {
        ...
        const country = trade.country || '';
        const current = costByCountry.get(country) ?? 0;
        costByCountry.set(country, current + investedPln);
      }
    }

    if (costByCountry.size === 0) return [];

    const total = [...costByCountry.values()].reduce((s, v) => s + v, 0);
    if (total <= 0) return [];

    const sorted = [...costByCountry.entries()].sort((a, b) => b[1] - a[1]);
    ...
    for (const [country, value] of sorted) {
      if (country && country !== 'XX' && value / total >= MIN_SHARE) {
        result.push({
          label: country,
          value,
...
<div class="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
  <h3 class="mb-1 text-sm font-semibold text-slate-700 dark:text-slate-300">
    {m.dash_country_diversification()}
  </h3>
